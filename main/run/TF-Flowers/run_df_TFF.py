...
def get_params_odfdf(coding_method, dict_size):
    if coding_method == 'svc':
        ...
        params_odfdf = dict(dict_size=dict_size, dict_method='Kmeans', reset=True, norm=True)
...
def get_spr_config(coding_method):
    if coding_method == 'hv':
        return dict(h_sp=[1, 2, 4], w_sp=[1, 2, 4], pooling_method='avg')
    else:
        return dict(h_sp=[1, 2, 4], w_sp=[1, 2, 4], pooling_method='max')
...
def by_cp_resnet50(coding_method, dict_size, group_size=2048, num_comp=-1, return_data=False):
    ...
    return obtain_cls_scores_for_CPFC('TF-Flowers', get_classifier_config(coding_method), params_oiv, params_odfdf, params_of, params_ottc,
...
def by_fc_resnext50_refined(coding_method, dict_size, group_size=2048, num_comp=-1, return_data=False, b_refined=False, b_weighted=False, beta=1):
    ...
    if params_odfdf['b_refined'] and params_odfdf['b_weighted']:
        params_odfdf['estimator_path'] = data_path + r'\intermediate_data\ext_feas\ext_dfs\ld\forests\resnext50(refined)\TF-Flowers\forests_kt256_mc10_md-1_ts450.pkl'

    coding_config = get_coding_config(coding_method)
    # coding_config['K_reset'] = params_odfdf['reset']
    coding_config['K_reset'] = True
    params_oiv = dict(num_comp=num_comp, coding_config=coding_config, spr_config=get_spr_config(coding_method), reset=False, skip=False)
