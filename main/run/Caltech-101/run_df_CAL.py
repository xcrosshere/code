...
def by_cp_resnet50(coding_method, dict_size, group_size=2048, num_comp=-1, return_data=False):
    ...
    return obtain_cls_scores_for_CPFC('Caltech-101', get_classifier_config(coding_method), params_oiv, params_odfdf, params_of, params_ottc,
...
def by_fc_vgg16bn_refined(coding_method, dict_size, group_size=4096, num_comp=-1, return_data=False):
    print('->' * 25, 'df:by_fc_vgg16bn_refined', '<-' * 25)
...
def by_fc_resnext50_refined(coding_method, dict_size, group_size=2048, num_comp=-1, return_data=False, b_refined=False, b_weighted=False, beta=1):
    ...
    if params_odfdf['b_refined'] and params_odfdf['b_weighted']:
        params_odfdf['estimator_path'] = data_path + r'\intermediate_data\ext_feas\ext_dfs\ld\forests\resnext50(refined)\Caltech-101\forests_kt2048_mc10_md9_ts30.pkl'
