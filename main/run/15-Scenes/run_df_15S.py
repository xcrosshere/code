...
def by_cp_resnet50(coding_method, dict_size, group_size=2048, num_comp=-1, return_data=False):
    ...
    params_of = dict(reset=True, skip=False, model_name='resnet50', target_layer='layer4.2.relu', group_size=group_size,
                     fea_source='cp')
    ...
    return obtain_cls_scores_for_CPFC('15-Scenes', get_classifier_config(coding_method), params_oiv, params_odfdf, params_of, params_ottc,
...
def by_fc_resnet50_refined(coding_method, dict_size, group_size=2048, num_comp=-1, return_data=False):
    ...
    params_of = dict(reset=False, skip=False, model_name='resnet50(refined)', target_layer='avgpool', group_size=group_size,
                     fea_source='fc', sizes=[96, 128, 160, 192, 224, 256], steps=[32, 32, 32, 32, 32, 32])
    params_odfdf = get_params_odfdf(coding_method, dict_size)
    coding_config = get_coding_config(coding_method)
    coding_config['K_reset'] = params_odfdf['reset']
...
def by_fc_resnext50_refined(coding_method, dict_size, group_size=2048, num_comp=-1, return_data=False, b_refined=False, b_weighted=False, beta=1):
    print('->' * 25, 'df:by_fc_resnext50_refined', '<-' * 25)

    params_of = dict(reset=False, skip=True, model_name='resnext50(refined)', target_layer='avgpool', group_size=group_size,
                     fea_source='fc', sizes=[96, 128, 160, 192, 224, 256], steps=[32, 32, 32, 32, 32, 32])
    params_odfdf = get_params_odfdf(coding_method, dict_size)
    params_odfdf['b_refined'] = b_refined
    params_odfdf['b_weighted'] = b_weighted
    params_odfdf['beta'] = beta
    if params_odfdf['b_refined'] and params_odfdf['b_weighted']:
        params_odfdf['estimator_path'] = data_path + r'\intermediate_data\ext_feas\ext_dfs\ld\forests\resnext50(refined)\15-Scenes\forests_kt256_mc10_md-1_ts100.pkl'

    coding_config = get_coding_config(coding_method)
    coding_config['K_reset'] = params_odfdf['reset']
    params_oiv = dict(num_comp=num_comp, coding_config=coding_config, spr_config=get_spr_config(coding_method), reset=True, skip=False)
...
if __name__ == '__main__':
    # by_cp_resnet50('sv', 4096, group_size=2048, num_comp=-1)
    # by_cp_swinb('sv', 16, group_size=1024, num_comp=-1)
    by_fc_swinb('fv', 1)
    # by_cp_resnet50('sv', -1)
