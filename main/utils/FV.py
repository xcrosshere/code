...
def dictionary(descriptors, N):
	...
	gmm.fit(descriptors)
	return float32(gmm.means_),float32(gmm.covariances_),float32(gmm.weights_)

def likelihood_moment(x, gaussians, weights, k, moment):
	x_moment = power(float32(x), moment) if moment > 0 else float32([1])
	probabilities = list(map(lambda i: weights[i] * gaussians[i], range(0, len(weights))))
...
def likelihood_statistics(samples, means, covs, weights):
	...
	gaussians = {}
	g = [multivariate_normal(mean=means[k], cov=diag(covs[k])) for k in range(0, len(weights)) ]
	for i,x in samples:
	...
	for k in range(0, len(weights)):
		...
		s0[k] = reduce(lambda a, ix: a + likelihood_moment(ix[1], gaussians[ix[0]], weights, k, 0), samples, 0)
		s1[k] = reduce(lambda a, ix: a + likelihood_moment(ix[1], gaussians[ix[0]], weights, k, 1), samples, 0)
		s2[k] = reduce(lambda a, ix: a + likelihood_moment(ix[1], gaussians[ix[0]], weights, k, 2), samples, 0)
	return s0, s1, s2
...
def fisher_vector(samples, means, covs, w):
	...
	T = samples.shape[0]
	covs = float32([covs[k] for k in range(0, covs.shape[0])])
	...
	# c = normalize(c)
	fv = concatenate([b,c])
	# fv = concatenate([concatenate(b), concatenate(c)])
	fv = normalize(fv)
	return fv